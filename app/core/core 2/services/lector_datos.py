...
class LectorDatosCSV:
    ...
    def cargar_datos(self) -> Tuple[List[Aspirante], List[Carrera]]:
        """Carga datos usando las rutas definidas en la normativa."""
        rutas = self.normativa.rutas
        return self.cargar_datos_desde_rutas(
            rutas['oferta_academica'],
            rutas['matriz_postulaciones']
        )

    def cargar_datos_desde_rutas(
        self, 
        ruta_oferta: str, 
        ruta_postulaciones: str
    ) -> Tuple[List[Aspirante], List[Carrera]]:
        """
        Carga datos desde rutas específicas.
        
        Args:
            ruta_oferta: Ruta al archivo CSV de oferta académica
            ruta_postulaciones: Ruta al archivo CSV de postulaciones
            
        Returns:
            Tupla con lista de aspirantes y lista de carreras
        """
        # Leer oferta académica
        try:
            df_oferta = pd.read_csv(ruta_oferta, dtype=str).fillna(0)
        ...
        try:
            df_post = pd.read_csv(ruta_postulaciones, dtype=str).fillna("")
        except Exception as e:
